...
const StudentReports = () => {
  ...
  const getReportComponents = (texto) => {
    try {
      let components = parseChatComponents(texto)
      return (
        <p>{components?.assistant.slice(0, 150)} ...</p>
      )
    } catch (error) {
      texto.slice(0, 100)
    }
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        {reports.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {reports
              .slice() // Crear una copia del array para no mutar el estado original
              .sort((a, b) => {
                // Ordenar por fecha (descendente) y luego por ID (ascendente)
                const dateDiff = a.id - b.id;
              ...
              .map((report) => (
                ...
                >
                  ...
                  <p>
                    {getReportComponents(report.texto_reporte)} ...
                  </p>
